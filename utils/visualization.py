...
class WriterTensorboardX:
    ...
    def __getattr__(self, name):
        ...
        if name in self.tb_writer_ftns:
            ...
            return attr
    def write_image(self, tar, prd, step, mode='train'):
        self.set_step(step, mode)
        if torch.is_complex(tar):
            tar = torch.clamp(torch.abs(tar),0,1)
        if torch.is_complex(prd):
            prd = torch.clamp(torch.abs(prd),0,1)
        self.add_image('tar', make_grid(tar, nrow=4, normalize=True))   
        self.add_image('prd', make_grid(prd, nrow=4, normalize=True))
        
    def write_data(self, data, step, mode='train'):
        self.set_step(step, mode)
        for key in data.keys():
            self.add_scalar(key, data[key])